...
class CrossSectional():
    ...
    def __init__(self, base_data, style_factors=pd.DataFrame(), industry_factors=pd.DataFrame()):
        ...
        self.stocknames = list(base_data.code)                                     # 股票名
        self.capital = base_data.captial.values                                    # 市值
    ...
    def reg(self):
        ...
        if self.P > 0:
            ...
            try:
                pure_factor_portfolio_weight = R @ np.linalg.inv(
                    factors_tran.T @ W @ factors_tran) @ factors_tran.T @ W  # 纯因子组合权重
            except:
                pure_factor_portfolio_weight = R @ np.linalg.pinv(
                    factors_tran.T @ W @ factors_tran) @ factors_tran.T @ W
...
#
#
#     csdata = CrossSectional(base_data, style_factors, industry_factors)
